...
#will then display in 2/3d plot



#Decreasing dimension to see if fixes concerns
VarDict= {'iotas': multi_iotas,
          'nfps':  multi_nfps ,
          'dominant nfps':  multi_dominant_nfps ,
          'helicities':  multi_helicities,
...
#A=[    matlab.double(np.ndarray.tolist(multi_max_curvatures) ),
#    matlab.double(np.ndarray.tolist(multi_max_elongations) ),
    # matlab.double( np.ndarray.tolist(multi_iotas)),
    # matlab.double(np.ndarray.tolist(multi_nfps) )
#]

#Y is the list of all the variables,
#A is the ploting variables
eng.FullHelo(A,Y, len(multi_iotas),False, nargout=0)
eng.title('std of Z vs std of R',nargout=0)
